...
public sealed class BarAggregatorMaskTests
{
    ...
    /// chiude alle 22:00 di Berlino = 21:00 UTC e riapre alle 00:15 UTC.
    /// </summary>
    [Fact]
    public void MinutesOutsideTheEurexWindowDoNotEnterTheFdaxBuckets()
    {
        ...
        var input = Minutes(Utc("2026-01-14T19:00:00Z"), Utc("2026-01-14T21:00:00Z"), 100m)   // dentro, bucket 20:00Z
            .Concat(Minutes(Utc("2026-01-14T21:00:00Z"), Utc("2026-01-15T00:15:00Z"), 500m))   // notte del CFD: fuori
        ...
        Assert.Equal(195, aggregator.MaskedMinutes);   // 21:00 → 00:15 = 3h15m
        Assert.Equal(2, bars.Count);

        var evening = bars[0];
        Assert.Equal(Utc("2026-01-14T20:00:00Z"), evening.Bar.DateTime);   // bucket 21:00-01:00 Roma
        Assert.Equal(60, evening.MinuteCount);                             // solo 21:00-22:00 Roma
    ...
    public void OverlapKeepsTheBucketThatOpensBeforeTheWindowAndDropsTheDeadHour()
    {
        ...
        Assert.False(mask.Overlaps(Utc("2026-01-14T21:00:00Z"), 60));   // 22:00 Roma, ora nativa
        ...
        Assert.Equal([Utc("2026-01-14T20:00:00Z"), Utc("2026-01-15T00:00:00Z")], kept.Select(bar => bar.DateTime));
