...
//   trades-cbot-*.json          trade del run cBot (esportati dalla console)
//   trades-interno-*.json       trade del backtest interno
//   backtest-summary-*.json     summary del backtest interno (fillConventions, planUniverse)
//   log.txt                     log del cBot (facoltativo: senza, saltano spread, esiti e intent)
//   *.xlsx                      export "Events" di cTrader (facoltativo: verifica delle uscite)
// Il feed a barre viene da piootoo-repository/datafeed-external/{broker}/, risalendo dalla
// cartella compare; il broker si legge da run-cbot-*.json (PriceSource.Broker) se non e' passato.
// La finestra di confronto e' la sovrapposizione degli ingressi delle due gambe.
//
// Scrive nella cartella di output (default <compare>/analisi): report.md, trade-abbinati.csv,
// trade-non-abbinati.csv, strategie.csv. Nato per compare-0033 (11/09/2026): la lettura delle
// sezioni sta in compare-0033/analisi-2026-09-11.md.
// ============================================================================================
...
var Cmp = Path.GetFullPath(args[0]);
var outDir = args.Length > 1 ? args[1] : Path.Combine(Cmp, "analisi");
Directory.CreateDirectory(outDir);

var repositoryRoot = FindRepositoryRoot(Cmp);
var broker = args.Length > 2 ? args[2] : ReadBrokerFromRun(Cmp) ?? "FTMO";
var Feed = Path.Combine(repositoryRoot, "datafeed-external", broker);
if (!Directory.Exists(Feed))
{
    Console.Error.WriteLine($"Feed non trovato: {Feed}");
    return 2;
}
...
static Dictionary<string, decimal>? LoadContractMultipliers(string repositoryRoot, JsonDocument summary)
{
